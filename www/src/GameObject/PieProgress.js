...
var PieProgress = function(game, x, y, radius, color, angle, weight)
{
    ...
    this.bmp = game.add.bitmapData((this._radius * 2) + (this._weight * (this._radius * 0.5)), (this._radius * 2) + (this._weight * (this._radius * 0.5)));
...
PieProgress.prototype.updateProgress = function() {
    ...
    this.bmp.ctx.arc(this.bmp.width * 0.5, this.bmp.height * 0.5, this._radius - 15, 0, (Math.PI * 2) * progress, true);
    this.bmp.ctx.stroke();
    this.bmp.dirty = true;
};
